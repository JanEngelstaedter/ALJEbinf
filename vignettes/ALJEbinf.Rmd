...
(The table could contain many NAs in those columns though, and can also contain additional column.)

Suppose we have such a table of mutations in the rpoB gene from the same three species as above, e.g.:
...
```{r}
filledMuts <- fillMutationsTable(muts, refs, seqs, coordinates)
filledMuts
```
